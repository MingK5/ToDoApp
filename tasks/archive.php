...
$validSorts = [
    ...
    'updated_time' => 't.updated_at DESC' // Renamed from updated_at to updated_time
];
...
$page = min($page, $totalPages ? $totalPages : 1); // Ensure page is valid
$start = ($page - 1) * $limit;
        ...
        <?php foreach ($tasks as $task): ?>
            <div class="card mb-2">
                <div class="card-body">
                    ...
                    <!-- Delete and unarchive forms -->
                    <form method="post" action="/ToDoApp/tasks/delete_task.php" class="d-inline">
                        <input type="hidden" name="task_id" value="<?= htmlspecialchars($task['id']) ?>">
                        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                        <button class="btn btn-danger btn-sm mt-2 me-2" type="submit" onclick="return confirm('Are you sure you want to delete this task?')">Delete</button>
                    </form>
                    <form method="post" action="/ToDoApp/tasks/unarchive_task.php" class="d-inline">
                        <input type="hidden" name="task_id" value="<?= htmlspecialchars($task['id']) ?>">
                        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                        <button class="btn btn-success btn-sm mt-2" type="submit" onclick="return confirm('Are you sure you want to unarchive this task?')">Unarchive</button>
                    </form>
                </div>
            </div>
        <?php endforeach; ?>
